...
export default function SigningBoardPage() {
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = "#070235"; // Navy color
    ctx.lineWidth = 3;
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="w-full max-w-2xl bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-8 md:p-12 text-center relative">
          ...
          <div className="w-20 h-20 bg-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-[0_0_30px_rgba(16,185,129,0.3)] animate-bounce">
            <span className="material-symbols-outlined text-[48px] text-slate-950 font-black">check</span>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-8 max-w-md mx-auto text-left space-y-2">
            ...
            <div className="flex justify-between text-xs text-slate-500">
              ...
              <span className="font-semibold text-emerald-400 uppercase flex items-center gap-1">
                <span className="material-symbols-outlined text-[14px]">lock</span> Secured & Encrypted
              </span>
            </div>
          </div>
          ...
          {revisionSent ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end gap-3">
                ...
                >
                  Send Revision <span className="material-symbols-outlined text-[16px]">send</span>
                </button>
              </div>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-[#070235]/5 bg-surface-bright flex flex-col items-center py-10 px-4">
      {/* Top Banner Context */}
      <header className="w-full max-w-4xl flex items-center justify-between mb-8 pb-4 border-b border-outline-variant/60">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#070235] text-[#85f8c4] rounded-lg flex items-center justify-center shadow-md">
            <span className="material-symbols-outlined">shield_lock</span>
          </div>
          <div>
            <h1 className="text-lg font-black text-[#070235] tracking-tight">TradeConnect Secure Deal Board</h1>
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Global B2B Institutional Escrow Board</p>
          </div>
        </div>
        <div className="bg-[#85f8c4]/30 text-[#070235] border border-[#85f8c4] px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm">
      ...
      <main className="w-full max-w-4xl bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-xl p-6 md:p-12 relative overflow-hidden mb-8">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-[0.02] pointer-events-none">
          <span className="text-[140px] font-black rotate-[-30deg] tracking-widest text-[#070235]">GLOBAL SECURE</span>
        </div>
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-outline-variant pb-6">
            <div>
              <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
            ...
            <div className="text-right flex flex-col items-end">
              <div className="w-12 h-12 bg-indigo-50 border border-outline-variant text-[#070235] rounded-xl flex items-center justify-center mb-2 shadow-inner">
                <span className="material-symbols-outlined text-2xl">domain</span>
              </div>
              <h3 className="text-base font-bold text-[#070235]">GlobalTech Imports GmbH</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-surface p-5 rounded-xl border border-outline-variant shadow-inner">
            ...
            <div className="border-t md:border-t-0 md:border-l border-outline-variant pt-6 md:pt-0 md:pl-8">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Shipping & Payment Terms</span>
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody className="text-xs md:text-sm">
                ...
                <tr className="border-b border-outline-variant/40">
                  ...
                  <td className="py-4 text-right font-black font-mono text-[#070235]">{formatCurrency(shippingTotal)}</td>
                </tr>
              </tbody>
              <tfoot>
                <tr>
                  <td colSpan={3}></td>
                  <td className="py-5 text-right font-bold text-slate-500 uppercase text-[10px] tracking-wider">Grand Total (USD)</td>
          ...
          {/* Signature Panel */}
          <div>
            <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-1.5">
            ...
            <div className="bg-surface border border-outline-variant rounded-xl p-6 shadow-inner">
              ...
              <div className="flex gap-2 mb-6 bg-surface-container-low p-1 rounded-lg w-fit border border-outline-variant">
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-1.5 ${signatureMode === "draw" ? "bg-[#070235] text-white shadow-sm" : "text-on-surface-variant hover:text-primary"}`}
                >
                  <span className="material-symbols-outlined text-[16px]">gesture</span> Draw Signature
                </button>
                <button 
                  onClick={() => setSignatureMode("type")}
                  className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-1.5 ${signatureMode === "type" ? "bg-[#070235] text-white shadow-sm" : "text-on-surface-variant hover:text-primary"}`}
                >
                  <span className="material-symbols-outlined text-[16px]">text_fields</span> Type Signature
                </button>
              </div>
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="relative w-full max-w-lg bg-surface-container-lowest border-2 border-dashed border-outline-variant rounded-xl overflow-hidden shadow-inner h-40">
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1">
                      <span className="text-[9px] font-bold bg-[#070235]/15 text-[#070235] px-2 py-0.5 rounded tracking-widest uppercase">Secure Pad</span>
                    </div>
                  </div>
                  <div className="flex gap-3 justify-end w-full max-w-lg">
                    ...
                    >
                      <span className="material-symbols-outlined text-[14px]">refresh</span> Clear Pad
                    </button>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-4">
                  <div className="flex flex-col gap-1 w-full max-w-lg">
                    ...
                    <input 
                      ...
                      className="w-full p-2.5 bg-surface-container-lowest border border-outline-variant rounded-md text-sm text-[#070235] font-semibold outline-none focus:border-primary"
                  ...
                  <div className="w-full max-w-lg bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col items-center justify-center text-center shadow-inner h-28">
                    <div className="font-[cursive] text-4xl text-[#070235] select-none opacity-80">
                      {typedName || "Your Signature"}
                    </div>
                    <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mt-3">Pre-rendered B2B Cursive Signature</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="pt-6 border-t border-outline-variant flex flex-col sm:flex-row justify-between items-center gap-4">
            <button 
              ...
              className="w-full sm:w-auto px-6 py-3 border border-outline-variant hover:border-amber-500 hover:text-amber-600 text-on-surface-variant font-bold text-sm rounded-xl transition-colors flex justify-center items-center gap-2"
            >
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button 
                ...
                className="w-full sm:w-auto px-8 py-3.5 bg-[#070235] text-white hover:bg-surface-tint font-bold text-sm rounded-xl transition-all flex justify-center items-center gap-2 shadow-lg"
              >
